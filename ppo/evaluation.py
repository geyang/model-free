...
from ppo.a2c_ppo_acktr import utils
...
def evaluate(actor_critic, obs_rms, env_name, seed, num_processes, eval_log_dir, device):
    ...
    # todo: add Args.eval_num_rollout
    for i in range(10):
        with torch.no_grad():
        ...
        for info in infos:
            if 'episode' in info.keys():
                logger.store_metrics({'test/reward': info['episode']['r']})

    eval_envs.close()

    # print(f" Evaluation using {len(eval_episode_rewards)} episodes: "
